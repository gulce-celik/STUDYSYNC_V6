...
public class DatabaseInspector {
    public static void main(String[] args) {
        ...
        try (Connection conn = DriverManager.getConnection(url, user, password);
            ...
            // 3. Inspect user_accounts columns
            System.out.println("\n--- Table Schema for user_accounts ---");
            try (ResultSet rs = stmt.executeQuery("SELECT * FROM user_accounts LIMIT 1")) {
                ResultSetMetaData metaData = rs.getMetaData();
                int columnCount = metaData.getColumnCount();
                for (int i = 1; i <= columnCount; i++) {
                    System.out.printf("Column %d: %s (%s)%n", i, metaData.getColumnName(i), metaData.getColumnTypeName(i));
                ...
                System.err.println("Error inspecting user_accounts schema: " + e.getMessage());
            }
